...
import { EntryInfo } from "../utils/types.ts";
import html from "../utils/html.ts";

export default function dirPage(dirname: string, entries: EntryInfo[]): string {
  return html`
    <!DOCTYPE html>
    <html lang="en">
      <head>
    ...
    entries.map(
      (entry) =>
        html`
        <li><a href="${entry.url}" title="${entry.name}" class="${entry.type} ${
